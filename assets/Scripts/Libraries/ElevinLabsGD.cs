...
public partial class ElevinLabsGD : Node
{
	...
	public async Task RenderVoice(string text)
	{
		if (string.IsNullOrEmpty(currentVoice))
		{
			...
			GetNode<NotificationsManager>("/root/Managers/Notifications").NewNotification("Error", "[center]Voice Error!", "[center]Please select a voice!", 5);
			return;
		}
		...
		try
		{
			...
			GetNode<NotificationsManager>("/root/Managers/Notifications").NewNotification("info", "[center]Generating Voice!", "[center]11 labs is now generating the voice!", 3);
		...
		catch (HttpRequestException ex)
		{
			...
			GetNode<NotificationsManager>("/root/Managers/Notifications").NewNotification("Error", "[center]Network Error!", "[center]Please check your connection.", 10);
		}
		catch (Exception ex)
		{
			if (ex.Message.Contains("invalid_api_key"))
			{
				...
				GetNode<NotificationsManager>("/root/Managers/Notifications").NewNotification("Error", "[center]Invalid API Key!", "[center]Please check your API key and try again.", 10);
			}
			else if (ex.Message.Contains("rate_limit_exceeded"))
			{
				GD.PrintErr("Rate limit exceeded. Please wait and try again later.");
				GetNode<NotificationsManager>("/root/Managers/Notifications").NewNotification("Error", "[center]Rate Limit Exceeded!", "[center]Please wait and try again later.", 10);
			}
			else
			{
				GD.PrintErr($"General error: {ex.Message}");
				GetNode<NotificationsManager>("/root/Managers/Notifications").NewNotification("Error", "[center]An Error Occurred!", "[center]Please try again.", 10);
			}
		}
	}
	...
	public async Task<Voice[]> GetVoices()
	{
		try
		{
			...
			{
				GetNode<NotificationsManager>("/root/Managers/Notifications").NewNotification("Error", "[center]Voices Failed", "[center]Voices failed to load. This may be due to an invalid API key.", 10);
				return null;
			}
			...
			GetNode<NotificationsManager>("/root/Managers/Notifications").NewNotification("Info", "[center]Voices Loaded", "[center]All voicebanks have successfully loaded", 10);
		...
		catch (Exception ex)
		{
			...
			{
				GD.PrintErr("Invalid API key. Please check your API key and try again.");
				GetNode<NotificationsManager>("/root/Managers/Notifications").NewNotification("Error", "[center]11labs Error!", "[center]Invalid API Key! Please check your API key and try again.", 10);
			}
			else if (ex.Message.Contains("rate_limit_exceeded"))
			{
				GD.PrintErr("Rate limit exceeded. Please wait and try again later.");
				GetNode<NotificationsManager>("/root/Managers/Notifications").NewNotification("Error", "[center]11labs Error!", "[center]Rate Limit Exceeded!, Please wait and try again later.", 10);
			}
			else
			{
				GD.PrintErr($"General error: {ex.Message}");
				GetNode<NotificationsManager>("/root/Managers/Notifications").NewNotification("Error", "[center]11labs Error!", "[center]An Error Occurred! Please try again.", 10);
			}
			return null;
		}
	}
